...
public class BookingSearch {
    ...
    static ArrayList<String[]> bookingSearch(String customerName) {
        if (conn != null) {
            try {
                ...
                String query = "SELECT datereserv, dateentree, nbrjour, nomclient, mail, datesortie, STRING_AGG(numchambr, ', ' ORDER BY numchambr) AS chambres FROM reserver WHERE dateentree >= ? AND idreserv NOT IN (SELECT idreserv FROM occuper) AND LOWER(nomclient) LIKE LOWER(?) GROUP BY nomclient, mail, datereserv, dateentree, nbrjour, datesortie ORDER BY dateentree ASC";
    ...
    static ArrayList<String[]> bookingSearch(String customerName, Date startDate) {
        if (conn != null) {
            try {
                ArrayList<String[]> listData = new ArrayList<>();
                 // Préparer la requête SQL avec des paramètres
    ...
    static ArrayList<String[]> bookingSearch(Date startDate) {
        if (conn != null) {
            try {
                ArrayList<String[]> listData = new ArrayList<>();
                 // Préparer la requête SQL avec des paramètres
